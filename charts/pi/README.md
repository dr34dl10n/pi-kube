...
docker build --target slim -t ghcr.io/dr34dl10n/pi-kube:<chartVersion> -f docker/Dockerfile.pi .
# full (batteries included)
docker build --target full -t ghcr.io/dr34dl10n/pi-kube:<chartVersion>-full -f docker/Dockerfile.pi .
# custom (your own apt packages)
docker build --build-arg PACKAGES="terraform helm kubectl" \
             -t myorg/pi-kube:<chartVersion>-custom -f docker/Dockerfile.pi .
```

Select with `image.tag` (e.g. `<chartVersion>-full`; empty resolves to the chart
version) or point `image.repository` at your custom image.
